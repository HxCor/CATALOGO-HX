...
const PASSWORD_PREFIX = "hmac-sha256-v1";
const ALLOWED_ORIGINS = new Set(["https://hxcor.github.io"]);
...
async function requireSession(request, env, adminOnly = false) {
  ...
  if (adminOnly && session.rol !== "admin") return { response: fail(request, 403, "Acceso restringido") };
  return { session };
}
...
function parseAllowedRfcs(value) {
  return [...new Set(String(value || "").split(/[\s,;]+/).map(v => v.trim().toUpperCase()).filter(Boolean))];
}
...
async function prepareUserFieldsForWrite(env, fields, isCreate) {
  ...
  if (password) {
    clean["Password Hash"] = await hashPassword(env, password);
    clean["Contraseña"] = "";
  } else {
...
async function handleUsers(request, env) {
  ...
  if (request.method === "POST") {
    let fields;
    try { fields = await prepareUserFieldsForWrite(env, body.fields, true); }
    ...
    return json(request, { ok: true, data: safe, record: safe }, 201);
  }

  if (request.method === "PUT" || request.method === "PATCH") {
    if (!body.recordId) return fail(request, 400, "recordId requerido");
    let fields;
    try { fields = await prepareUserFieldsForWrite(env, body.fields, false); }
    catch (e) { return fail(request, 400, e.message); }
    const updated = await airtableRequest(env, table, { method: "PATCH", recordId: body.recordId, body: { fields } });
    const safe = publicUser(updated);
    return json(request, { ok: true, data: safe, record: safe });
  }
...
function filterProvidersForSession(records, session) {
...
async function allowedProviderIds(env, session) {
  ...
  return new Set(filterProvidersForSession(await listAllRecords(env, table), session).map(r => r.id));
}
...
async function handleGenericCrud(request, env, type) {
  ...
  const table = envTable(env, tableKey, type === "proveedores" ? "Proveedores" : "BANCOS");

  if (request.method === "GET") {
    let records = await listAllRecords(env, table);
    if (type === "proveedores") {
      records = filterProvidersForSession(records, session);
  ...
  if (request.method === "PUT" || request.method === "PATCH") {
    if (!body.recordId) return fail(request, 400, "recordId requerido");
    const updated = await airtableRequest(env, table, { method: "PATCH", recordId: body.recordId, body: { fields: body.fields || {} } });
