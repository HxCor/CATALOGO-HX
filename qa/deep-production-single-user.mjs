...
const DOCS=process.env.DOCS_TABLE_ID||'tblgJzeG5asIqC3EA';
...
const quoteIds=[];const documentIds=[];let testId='',qaProviderId='',browser=null,failed=0;
...
async function waitForLiveRelease(){
 ...
 const expected=[...local.matchAll(/(?:app\.html|security-post\.js|expediente-loader\.js)\?v=[^"'\s<]+/g)].map(m=>m[0]);
 if(expected.length<3)throw new Error('local_release_markers_missing');
...
async function testExpedienteUi(page,label,minimumCards){
 await page.locator('#cardsGrid .pcard').first().click();
 await page.locator('#hxExpedienteSection').waitFor({state:'visible',timeout:20000});
 await page.waitForFunction(()=>{const b=document.querySelector('#hxExpedienteBody');return b&&!b.classList.contains('hx-expediente-loading');},null,{timeout:30000});
 const state=await page.evaluate(()=>({cards:document.querySelectorAll('#hxExpedienteBody .hx-doc-card').length,loading:document.querySelector('#hxExpedienteBody')?.classList.contains('hx-expediente-loading'),previewLoaded:!!document.querySelector('#hxPreviewOverlay'),uploadLoaded:!!document.querySelector('#hxUploadOverlay')}));
 if(state.loading||state.cards<minimumCards||!state.previewLoaded||!state.uploadLoaded)throw new Error(`${label}_expediente_ui_${JSON.stringify(state)}`);
 pass(`${label}_UI_EXPEDIENTE`,`cards_${state.cards}`);
 await page.locator('#detailOverlay .modal-close').click();
}
...
try{
 ...
 for(const asset of ['expediente-loader.js','expediente-digital.js','divisas-hx-pro.js','divisas-hx-pro-fix.js','laboral-hx.js','laboral-imss.js','laboral-access.js','laboral-permissions-fix.js','laboral-navigation-fix.js']){const r=await req(`${SITE}${asset}?deep=${Date.now()}`);r.status===200?pass(`ASSET_${asset.replace(/[^A-Za-z0-9]/g,'_').toUpperCase()}`):fail(`ASSET_${asset}`,`http_${r.status}`);}
 ...
 await setRole('viewer');pass('QA_ROLE_VIEWER_SET');let l=await loginApi();const viewerToken=l.json?.token;if(viewerToken&&l.json?.usuario?.rol==='viewer')pass('VIEWER_LOGIN_API');else throw new Error(`viewer_login_http_${l.status}`);const vu=await req(`${API}/usuarios`,{headers:auth(viewerToken)});vu.status===403?pass('VIEWER_USERS_BLOCKED'):fail('VIEWER_USERS_BLOCKED',`http_${vu.status}`);const vp=await req(`${API}/proveedores`,{headers:auth(viewerToken)});vp.status===200&&vp.json?.ok&&vp.json?.records?.length===1?pass('VIEWER_PROVIDER_SCOPE'):fail('VIEWER_PROVIDER_SCOPE',`count_${vp.json?.records?.length??'na'}`);qaProviderId=vp.json?.records?.[0]?.id||'';const ve=await req(`${API}/expedientes?proveedorId=${encodeURIComponent(qaProviderId)}`,{headers:auth(viewerToken)});ve.status===200&&ve.json?.ok&&Array.isArray(ve.json.records)&&ve.json.records.every(record=>!record.sensible)?pass('VIEWER_EXPEDIENTE_SCOPE'):fail('VIEWER_EXPEDIENTE_SCOPE',`http_${ve.status}`);const vef=new FormData();const veu=await req(`${API}/expedientes/upload`,{method:'POST',headers:auth(viewerToken),body:vef});veu.status===403?pass('VIEWER_EXPEDIENTE_WRITE_BLOCKED'):fail('VIEWER_EXPEDIENTE_WRITE_BLOCKED',`http_${veu.status}`);const vr=await req(`${API}/divisas/current`,{headers:auth(viewerToken)});vr.status===200&&vr.json?.ok&&Number(vr.json?.data?.average)>0?pass('VIEWER_DIVISAS_RATE'):fail('VIEWER_DIVISAS_RATE',`http_${vr.status}`);
 ...
 for(const [name,viewport] of [['VIEWER_DESKTOP',{width:1440,height:1000}],['VIEWER_MOBILE',{width:390,height:844}]]){
  ...
  try{
   ...
   await testExpedienteUi(p,name,3);await testModulesUi(p,name);
 ...
 await setRole('admin');pass('QA_ROLE_ADMIN_SET');l=await loginApi();const adminToken=l.json?.token;if(adminToken&&l.json?.usuario?.rol==='admin')pass('ADMIN_LOGIN_API');else throw new Error(`admin_login_http_${l.status}`);const ae=await req(`${API}/expedientes?proveedorId=${encodeURIComponent(qaProviderId)}`,{headers:auth(adminToken)});ae.status===200&&ae.json?.ok?pass('ADMIN_EXPEDIENTE_LIST'):fail('ADMIN_EXPEDIENTE_LIST',`http_${ae.status}`);const aef=new FormData();aef.append('proveedorId',qaProviderId);aef.append('tipo','QA_TEST_DOCUMENT');aef.append('file',new Blob(['%PDF-1.4\\n% HX QA expediente\\n'],{type:'application/pdf'}),'qa-expediente.pdf');const aeu=await req(`${API}/expedientes/upload`,{method:'POST',headers:auth(adminToken),body:aef});const documentId=aeu.json?.record?.id;if(documentId)documentIds.push(documentId);[200,201].includes(aeu.status)&&documentId?pass('ADMIN_EXPEDIENTE_UPLOAD'):fail('ADMIN_EXPEDIENTE_UPLOAD',`http_${aeu.status}_${String(aeu.json?.error||'').replace(/\\s+/g,'_')}`);if(documentId){const aed=await req(`${API}/expedientes/${documentId}/file`,{headers:auth(adminToken)});aed.status===200&&String(aed.headers.get('content-type')||'').includes('application/pdf')&&aed.text.includes('HX QA expediente')?pass('ADMIN_EXPEDIENTE_PRIVATE_VIEW'):fail('ADMIN_EXPEDIENTE_PRIVATE_VIEW',`http_${aed.status}`);}const au=await req(`${API}/usuarios`,{headers:auth(adminToken)});au.status===200&&au.json?.ok&&Array.isArray(au.json.records)?pass('ADMIN_USERS_API'):fail('ADMIN_USERS_API',`http_${au.status}`);const sensitive=JSON.stringify(au.json||{}).match(/Password Hash|Contraseña|TOTP Secret/gi)||[];sensitive.length===0?pass('ADMIN_USERS_API_SANITIZED'):fail('ADMIN_USERS_API_SANITIZED',`matches_${sensitive.length}`);const ar=await req(`${API}/divisas/current`,{headers:auth(adminToken)});ar.status===200&&ar.json?.ok?pass('ADMIN_DIVISAS_RATE'):fail('ADMIN_DIVISAS_RATE',`http_${ar.status}`);const ai=await req(`${API}/laboral/imss-cost`,{method:'POST',headers:auth(adminToken,{'Content-Type':'application/json'}),body:JSON.stringify({monthlySalary:30000,region:'general',serviceYear:1,vacationDays:12,aguinaldoDays:15,vacationPremiumPct:25,daysCotized:30.4,riskClass:'V'})});ai.status===200&&ai.json?.ok&&ai.json?.result?.assumptions?.riskClass==='V'?pass('ADMIN_IMSS_API'):fail('ADMIN_IMSS_API',`http_${ai.status}`);const ac=await browser.newContext({viewport:{width:1440,height:1000}});const ap=await ac.newPage();
 try{await browserLogin(ap);await ap.waitForFunction(()=>document.body.classList.contains('is-admin'),null,{timeout:20000});await ap.locator('#adminAddBtn').waitFor({state:'visible',timeout:15000});pass('ADMIN_UI_CONTROLS');await testExpedienteUi(ap,'ADMIN',5);await testCatalogOrderUi(ap,'ADMIN');await testModulesUi(ap,'ADMIN');}
