...
async function testModulesUi(page,label){
 ...
 const txt=await page.locator('#hxliResult').innerText();for(const m of ['SBC diario','INFONAVIT','Costo anual estimado','NO ES DETERMINACIÓN OFICIAL'])if(!txt.includes(m))throw new Error(`${label}_imss_missing_${m}`);const sbcVisible=await page.locator('#hxliResult .hxli-summary-card').filter({hasText:'SBC diario'}).isVisible().catch(()=>false);if(!sbcVisible)throw new Error(`${label}_sbc_not_visible`);pass(`${label}_UI_IMSS`);
}
try{
