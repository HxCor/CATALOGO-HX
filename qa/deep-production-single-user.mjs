...
async function waitForLiveRelease(){
 ...
 const expected=[...local.matchAll(/(?:app\.html|security-post\.js|expediente-digital\.js)\?v=[^"'\s<]+/g)].map(m=>m[0]);
...
try{
 ...
 for(const asset of ['expediente-digital.js','divisas-hx-pro.js','divisas-hx-pro-fix.js','laboral-hx.js','laboral-imss.js','laboral-access.js','laboral-permissions-fix.js','laboral-navigation-fix.js']){const r=await req(`${SITE}${asset}?deep=${Date.now()}`);r.status===200?pass(`ASSET_${asset.replace(/[^A-Za-z0-9]/g,'_').toUpperCase()}`):fail(`ASSET_${asset}`,`http_${r.status}`);}
