...
const runtime = read('security-runtime.js');
...
check('Plataforma', 'Carga principal reutiliza caché', index.includes("cache: 'default'"));
check('Plataforma', 'Assets con versión final', index.includes('final-20260824-1'));
check('Seguridad', 'Escape HTML disponible', runtime.includes('window.hxEscapeHtml'));
check('Seguridad', 'URLs externas validadas', runtime.includes('window.hxSafeHttpUrl'));
check('Seguridad', 'Sesión con renovación', worker.includes('handleSessionRefresh') && runtime.includes('startHxSessionRefresh'));
...
if (process.argv.includes('--live')) {
  ...
  await probe('Plataforma', 'GitHub Pages disponible', `https://hxcor.github.io/CATALOGO-HX/?audit=${nonce}`, 200, 'app.html?v=final-20260824-1');
